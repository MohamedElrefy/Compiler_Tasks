    ...
    def input_grammar(self):
        ...
        for nt in non_terminals:
            ...
            for i in range(1, 3):  # Two rules for each non-terminal
                rule = input(
                    f"Enter rule number {i} for non-terminal '{nt}' (use capital letters for non-terminals): ").strip()
                rules.append(rule)
    ...
    def is_simple_grammar(self):
        ...
        for nt, rules in self.grammar.items():
            ...
            if len(starting_symbols) != len(set(starting_symbols)):
                self.print_colored(f"The Grammar isn't simple: Multiple rules for '{nt}' start with the same symbol.",
                                   'WARNING')
                return False

            # Check for multiple rules leading to ambiguity
            if len(set(rules)) < len(rules):
                self.print_colored(f"The Grammar isn't simple: Multiple identical rules detected for '{nt}'.",
                                   'WARNING')
                return False
